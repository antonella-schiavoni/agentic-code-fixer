...
"""
        ...
        for patch in patches:
            ...
            if agent_id not in agent_stats:
                agent_stats[agent_id] = {'count': 0, 'avg_confidence': 0, 'total_confidence': 0}

            agent_stats[agent_id]['count'] += 1
            agent_stats[agent_id]['total_confidence'] += patch.get('confidence_score', 0)
...
"""
        ...
        if evaluations:
            ...
            if patch_wins:
                report += "\n### Wins by Patch\n"
                for patch_id, wins in sorted(patch_wins.items(), key=lambda x: x[1], reverse=True):
...
"""
            ...
            if test_results:
                avg_duration = sum(t.get('duration_seconds', 0) for t in test_results) / len(test_results)
...
"""
    ...
    def print_summary_table(self) -> None:
        ...
        table.add_row("Duration", f"{self.experiment_data.get('duration_seconds', 0):.1f}s")
        table.add_row("Patches Generated", str(len(patches)))
        table.add_row("Status", "SUCCESS" if self.experiment_data.get('success', False) else "FAILED")
        table.add_row("Winning Patch", self.experiment_data.get('winning_patch_id', 'None'))
    ...
    def export_statistics(self) -> dict[str, Any]:
        ...
        patch_stats = {
            ...
            "avg_confidence": sum(p.get('confidence_score', 0) for p in patches) / len(patches) if patches else 0,
            "patches_by_agent": {},
        }
        ...
        evaluation_stats = {
            ...
            "avg_evaluation_confidence": sum(e.get('confidence', 0) for e in evaluations) / len(evaluations) if evaluations else 0,
        }

        test_stats = {
            "total_tests": len(test_results),
            "tests_passed": sum(1 for t in test_results if t.get('passed', False)),
            "avg_test_duration": sum(t.get('duration_seconds', 0) for t in test_results) / len(test_results) if test_results else 0,
        }
